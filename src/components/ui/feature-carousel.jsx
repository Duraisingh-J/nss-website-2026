...
export function FeatureCarousel({ features = DEFAULT_FEATURES, themeColor = "#62B2FE", onSelect }) {
  ...
  const currentIndex =
    ((step % activeFeatures.length) + activeFeatures.length) % activeFeatures.length;
  ...
  const handleChipClick = (index) => {
    const diff = (index - currentIndex + activeFeatures.length) % activeFeatures.length;
    if (diff > 0) setStep((s) => s + diff);
    if (onSelect) {
      onSelect(activeFeatures[index]);
    }
  };
  ...
  const getCardStatus = (index) => {
    ...
    const len = activeFeatures.length;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto md:p-8">
      <div className="relative overflow-hidden rounded-[2.5rem] lg:rounded-[4rem] flex flex-col lg:flex-row min-h-[600px] lg:aspect-video border border-border/40 shadow-2xl bg-surface">
        <div
        ...
        >
          <div
            ...
            style={{
              background: `linear-gradient(to bottom, ${themeColor}, ${themeColor}CC, transparent)`,
            }}
          />
          <div
            className="absolute inset-x-0 bottom-0 h-12 md:h-20 lg:h-16 z-40 pointer-events-none"
            style={{
              background: `linear-gradient(to top, ${themeColor}, ${themeColor}CC, transparent)`,
            }}
          />
        ...
        <div className="flex-1 min-h-[500px] md:min-h-[600px] lg:h-full relative bg-slate-900/5 flex items-center justify-center py-16 md:py-24 lg:py-16 px-6 md:px-12 lg:px-10 overflow-hidden border-t lg:border-t-0 lg:border-l border-border/20">
              ...
              return (
                ...
                >
                  ...
                  <AnimatePresence>
                    {isActive && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute inset-x-0 bottom-0 p-8 md:p-10 pt-32 bg-gradient-to-t from-black/95 via-black/50 to-transparent flex flex-col justify-end pointer-events-none"
                      >
                        <div className="bg-white/95 text-slate-900 px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-[0.2em] w-fit shadow-lg mb-3 border border-white/20 backdrop-blur-sm">
                          {index + 1} • {feature.category || feature.label}
                        </div>
                        <p className="text-white font-medium text-lg md:text-2xl leading-snug drop-shadow-md tracking-tight">
                          {feature.description}
                        </p>
                      </motion.div>
                    )}
                  ...
                  >
                    ...
                    <span className="text-white/90 text-[10px] font-bold uppercase tracking-[0.3em] font-mono drop-shadow">
                      {feature.tag || "NSS Spotlight"}
                    </span>
                  </div>
                </motion.div>
              );
